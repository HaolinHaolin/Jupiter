...
public class GeoRecommendation {
	...
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		for (String favoriteItemId : favoritedItemIds) {
			Set<String> categories = connection.getCategories(favoriteItemId);
			for (String category : categories) {
				allCategories.put(category, allCategories.getOrDefault(category, 0) + 1);
			}
		}
		List<Entry<String, Integer>> categoryList = new ArrayList<Map.Entry<String,Integer>>(allCategories.entrySet());
		...
		// {music: 5, art: 3, sports: 1} 去重 搜索的结果
